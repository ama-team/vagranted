<?php

namespace AmaTeam\Vagranted\Application\Configuration;

/**
 * @author Etki <etki@etki.me>
 */
class Constants
{
    const RESOURCE_SET_CONFIGURATION_FILE = 'vagranted.yml';
    const INSTALLATION_SPECIFICATION_FILE = 'specification.yml';
    const INSTALLATION_METADATA_FILE = 'metadata.yml';
    const INSTALLATION_STATISTICS_FILE = 'statistics.yml';
    const INSTALLATION_ARTIFACT_DIRECTORY = 'artifact';
    const LOCAL_SCHEMAS = ['fs', 'file', 'local'];
}
